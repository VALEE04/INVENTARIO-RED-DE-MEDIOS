...
<table class="table table-bordered">
  ...
  <tbody>
    ...
    if (isset($_POST['enviar'])) {
        // Buscador funcional
        $buscador = htmlspecialchars($_POST['buscar'], ENT_QUOTES, 'UTF-8');
        $sql = "SELECT * FROM encargados WHERE 
                NOMBRE_ENCARGADO LIKE '%$buscador%' OR 
                ROL LIKE '%$buscador%' OR 
                email LIKE '%$buscador%' OR 
                numero_documento LIKE '%$buscador%' OR 
                telefono LIKE '%$buscador%' 
                ORDER BY ID_ENCARGADO ASC";
        ...
        while ($row = mysqli_fetch_assoc($query)) {
            ...
            <tr>
                ...
                <td>
                    <div class="d-flex">
                        <a href="actualizar_encargados.php?id=<?php echo $row['ID_ENCARGADO']; ?>" class="btn btn-primary btn-sm me-2">Editar</a>
                        <a href="ListaEncargados/eliminar_encargados.php?id=<?php echo $row['ID_ENCARGADO']; ?>" class="btn btn-danger btn-sm" onclick="return confirmDelete()">Eliminar</a>
                    </div>
                </td>
            </tr>
            <?php
