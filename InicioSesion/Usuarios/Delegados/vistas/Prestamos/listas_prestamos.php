...
<html lang="en">
<head>
    <title>Listas de Préstamos</title>
...
<body><br>
<center><h1>Lista de Préstamos</h1></center>
<br>
<div class="container-fluid">
    ...
    <form action="<?=$_SERVER['PHP_SELF']?>" method="post">
        ...
        <input class="btn btn-outline-info" type="hidden" name="enviar">
                ...
                <a href="listas_prestamos.php"> <button class="btn btn-outline-info" type="submit" name=""  >Mostrar todos los datos</button></a><br><br>
                <a href="../Prestamos/pdf" class="btn btn-outline-info" style="margin-right:850px;">Generar Reporte</a>
                <a href="registrar_prestamos.php"> <button type="button" class="btn btn-outline-info" style="  margin-right:-100px; "  >Añadir </button></a>
...
<table class="table table-bordered">
  ...
  <tbody>
  	...
  	if (isset($_POST['enviar'])) {
  		...
  		$sql="SELECT * FROM prestamo WHERE NOMBRE_ALMACENADO LIKE '%".$buscador."%' OR FECHA_ENTREGA LIKE '%".$buscador."%' OR FECHA_SALIDA LIKE '%".$buscador."%' OR NOMBRE_ENCARGADO LIKE '%".$buscador."%' OR ROL LIKE '%".$buscador."%' OR nodo LIKE '%".$buscador."%'  ";
  		$query=mysqli_query($con,$sql);
  		while($row=mysqli_fetch_assoc($query)){
  			?>
    <tr>
    	<td><?php echo $row['ID_FECHA']?></td>
        <td><?php echo $row['ID_ALMACENADO']?></td>
      <td><?php echo $row['NOMBRE_ALMACENADO']?></td>
      ...
      <td>
      <a href="actualizar_prestamos.php?id=<?php echo $row['ID_FECHA']?>" class="btn btn-primary">Editar</a>
      <a href="ListaPrestamos/eliminar_prestamos.php?id=<?php echo $row['ID_FECHA']?>" class="btn btn-danger" onclick="return confirmDelete()" >Eliminar</a></td>
      </td>
    </tr>
<?php
 }
...
<tr>
    ...
    <div class="d-flex justify-content-center">
        <a href="actualizar_prestamos.php?id=<?php echo $row['ID_FECHA']?>" class="btn btn-sm btn-primary me-2">Editar</a>
        <a href="ListaPrestamos/eliminar_prestamos.php?id=<?php echo $row['ID_FECHA']?>" class="btn btn-sm btn-danger" onclick="return confirmDelete()">Eliminar</a>
    </div>
</td>
    </tr>
    <?php
