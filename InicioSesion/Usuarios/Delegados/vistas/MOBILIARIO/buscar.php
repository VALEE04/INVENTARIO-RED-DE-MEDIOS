                ...
                if ($numero > 0) {
                    ...
                    while ($row = $result->fetch_assoc()) {
                        ...
                        <tr>
                            ...
                            <td><?= resaltar_frase($row['categoria_nombre'], $_POST['buscar']); ?></td>
                            <td><?= resaltar_frase($row['cantidad'], $_POST['buscar']); ?></td>
                            <td><?= resaltar_frase($row['estado'], $_POST['buscar']); ?></td>
                            <td><?= resaltar_frase($row['observaciones'], $_POST['buscar']); ?></td>
                            <td><?= resaltar_frase($row['asignado_a'], $_POST['buscar']); ?></td> <!-- Mostrar "Asignado a" -->
                            <td><img src="imagenes/<?= $row['Imagen_Img']; ?>" class="img-rounded" width="50px" height="50px" /></td>
                            <td>
                                <a class="btn btn-info" href="EditarImagen.php?edit_id=<?= $row['Imagen_ID']; ?>" title="Editar" onclick="return confirm('¿Está seguro de editar el archivo?')">
                                    <span class="glyphicon glyphicon-edit"></span> Editar
                                </a>
                                <a class="btn btn-danger" href="eliminar_mobiliario.php?id=<?= $row['Imagen_ID']; ?>" title="Eliminar" onclick="return confirm('¿Está seguro de eliminar el archivo?')">
                    ...
                    echo "<tr><td colspan='9' class='text-center' style='color: #555;'><strong>No se encontraron resultados.</strong></td></tr>";
                }
            } else {
                $stmt = $DB_con->prepare('SELECT Imagen_ID,descripcion,categoria_nombre,cantidad,estado,observaciones,asignado_a,Imagen_Img FROM mobiliario ORDER BY Imagen_ID DESC');
                $stmt->execute();
                ...
                if ($stmt->rowCount() > 0) {
                    ...
                    while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                        ...
                        <tr>
                            ...
                            <td><?= $row['asignado_a']; ?></td> <!-- Mostrar "Asignado a" -->
                            <td><img src="imagenes/<?= $row['Imagen_Img']; ?>" class="img-rounded" width="50px" height="50px" /></td>
                            <td>
                                <a class="btn btn-info" href="EditarImagen.php?edit_id=<?= $row['Imagen_ID']; ?>" title="Editar" onclick="return confirm('¿Está seguro de editar el archivo?')">
                                    <span class="glyphicon glyphicon-edit"></span> Editar
                    ...
                    ?>
                    <tr><td colspan="9" class="text-center" ><strong>No se encontraron resultados.</strong></td></tr>
                    <?php
                }
            }
            ?>
